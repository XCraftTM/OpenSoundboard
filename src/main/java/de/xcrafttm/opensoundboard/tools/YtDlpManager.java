 ...
 *
 * This downloads the binaries into the mod's working directory and then runs yt-dlp
...
public final class YtDlpManager {
    ...
    private static final boolean IS_WINDOWS = Util.getOperatingSystem().getName().toLowerCase(Locale.ROOT).contains("windows");
    ...
    private static File libDir() {
        File dir = new File(new File(net.fabricmc.loader.api.FabricLoader.getInstance().getConfigDir().toFile(), "opensoundboard"), "lib");
        if (!dir.exists()) {

...
public class YouTubeScreen extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout root) {
        ...
        var title = Components.label(Text.translatable("gui.opensoundboard.youtube.title").formatted(Formatting.BOLD));
        ...
        urlField = Components.textBox(Sizing.fixed(columnWidth));
        ...
        var actions = (FlowLayout) Containers.horizontalFlow(Sizing.fixed(columnWidth), Sizing.content())
                .gap(6)
                .verticalAlignment(VerticalAlignment.CENTER);

        audioToggle = Components.button(audioToggleLabel(), b -> {
            audioOnly = !audioOnly;
            b.setMessage(audioToggleLabel());
        });
        audioToggle.sizing(Sizing.fixed(150), Sizing.content());

        downloadBtn = Components.button(Text.translatable("gui.opensoundboard.youtube.download"), b -> {
        ...
        var folderBtn = Components.button(Text.translatable("gui.opensoundboard.folder"), b ->
        ...
        progressOuter = Containers.horizontalFlow(Sizing.fixed(columnWidth), Sizing.fixed(6));
        progressOuter.surface(Surface.flat(0xFF555555));

        progressInner = Containers.horizontalFlow(Sizing.fixed(0), Sizing.fixed(6));
        ...
        logList = Containers.verticalFlow(Sizing.fixed(columnWidth - 12), Sizing.content()).gap(1);
        logScroll = Containers.verticalScroll(Sizing.fixed(columnWidth), Sizing.fixed(Math.max(140, this.height - 240)), logList);
        logScroll.surface(Surface.flat(0x66000000)).padding(Insets.of(6));
        root.child(logScroll);

        addLog("> Waiting for Command...");

        var saveInfo = Components.label(Text.translatable("gui.opensoundboard.youtube.save_folder", OpenSoundboardClient.soundDir.getAbsolutePath())
        ...
        var doneBtn = Components.button(Text.translatable("gui.done"), b -> close());
    ...
    private void addLog(String line) {
        ...
        if (renderer == null) {
            logList.child(Components.label(Text.literal(line)).horizontalSizing(Sizing.fill(100)));
        ...
        while (!remaining.isEmpty()) {
            ...
            if (split <= 0 || split >= remaining.length()) {
                logList.child(Components.label(Text.literal(remaining)).horizontalSizing(Sizing.fill(100)));
                break;
            }

            String part = remaining.substring(0, split);
            logList.child(Components.label(Text.literal(part)).horizontalSizing(Sizing.fill(100)));

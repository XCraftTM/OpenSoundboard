...
public class SoundWheelOverlay extends BaseOwoScreen<FlowLayout> {
    ...
    public SoundWheelOverlay() {
        super(Component.literal("Sound Wheel"));
    }
    ...
    protected @NotNull OwoUIAdapter<FlowLayout> createAdapter() {
        return OwoUIAdapter.create((Screen) this, UIContainers::verticalFlow);
    }
    ...
    private void rebuildUI(FlowLayout root) {
        ...
        String pageText = allSounds.isEmpty()
                ...
                : Component.translatable("gui.opensoundboard.wheel.page",
        ...
        if (currentFolder != null) {
            var folderLabel = UIComponents.label(
                    Component.literal(currentFolder.getName()).withStyle(ChatFormatting.YELLOW));
        ...
        ButtonComponent centerBtn = UIComponents.button(Component.literal(pageText), b -> {});
        ...
        for (int i = 0; i < numButtons; i++) {
            ...
            if (i < sounds.size() && sounds.get(i) != null) {
                ...
                if (isBack) {
                    buttonText = Component.literal("← ").withStyle(ChatFormatting.GRAY)
                            .append(Component.literal(currentFolder != null ? currentFolder.getName() : "")
                                    .withStyle(ChatFormatting.GRAY));
                } else if (isFolder) {
                    String folderLabel = GuiTools.trimName(((Screen) this).textRenderer, "📁 " + file.getName(), BUTTON_W - 8);
                    buttonText = Component.literal(folderLabel).withStyle(ChatFormatting.YELLOW);
                } else {
                    ...
                    if (isFav) {
                        buttonText = Component.literal("★ ").withStyle(ChatFormatting.GOLD)
                                .append(Component.literal(label).withStyle(isPlaying ? ChatFormatting.GREEN : ChatFormatting.YELLOW));
                    } else {
                        buttonText = Component.literal(label).withStyle(isPlaying ? ChatFormatting.GREEN : ChatFormatting.WHITE);
                    }
                    if (isPlaying) buttonText.append(Component.literal(" ▶").withStyle(ChatFormatting.GREEN));
                }
                ...
                ButtonComponent filler = UIComponents.button(Component.empty(), b -> {});
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        conComponent.fill(0, 0, this.width, this.height, 0x66000000);

...
public class SoundboardScreen extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout root) {
        ...
        // Title
        root.child(UIComponents.label(Component.translatable("gui.opensoundboard.title").withStyle(ChatFormatting.BOLD))
                .horizontalTextAlignment(HorizontalAlignment.CENTER)
                .margins(Insets.bottom(5)));

        // Search
        queryField = UIComponents.textBox(Sizing.fixed(listWidth));
        queryField.setPlaceholder(Component.translatable("gui.opensoundboard.search_hint"));
        ...
        var topButtons = (FlowLayout) UIContainers.horizontalFlow(Sizing.fixed(listWidth), Sizing.content())
        ...
        sortModeBtn = UIComponents.button(sortModeLabel(), b -> {
        ...
        sortModeBtn.tooltip(Component.translatable("tooltip.opensoundboard.sortMode"));
        topButtons.child(sortModeBtn);

        sortDirBtn = UIComponents.button(sortDirLabel(), b -> {
        ...
        sortDirBtn.tooltip(Component.translatable("tooltip.opensoundboard.sortDir"));
        ...
        root.child(UIComponents.button(Component.translatable("gui.done"), b -> close())
    ...
    private void buildDetailsPane(FlowLayout root) {
        var pane = UIContainers.verticalFlow(Sizing.fixed(listWidth), Sizing.content());
        ...
        // Detail title
        detailLabel = UIComponents.label(Component.translatable("gui.opensoundboard.select_hint").withStyle(ChatFormatting.GRAY));
        ...
        var sliderRow = (FlowLayout) UIContainers.horizontalFlow(Sizing.fixed(innerW), Sizing.content())
        ...
        detailLocalSlider.message(v -> {
            ...
            return SoundboardConfig.data.isSyncAudio()
                    ? Component.translatable("gui.opensoundboard.sync_volume",  String.valueOf(pct))
                    : Component.translatable("gui.opensoundboard.local_volume", String.valueOf(pct));
        });
        ...
        detailPlayerSlider.message(v ->
                Component.translatable("gui.opensoundboard.player_volume", String.valueOf(pctFromSlider(v))));
        detailPlayerSlider.onChanged().subscribe(value -> updateSelectedVolume(null, value));

        detailBindBtn = (ButtonComponent) UIComponents.button(
                Component.translatable("gui.opensoundboard.keybind.none"), b -> {
                    if (selectedFile != null) {
                        isBinding = true;
                        b.setMessage(Component.translatable("gui.opensoundboard.keybind.listening").withStyle(ChatFormatting.YELLOW));
                    }
                }).sizing(Sizing.fixed(colW), Sizing.content());
        detailBindBtn.tooltip(Component.translatable("tooltip.opensoundboard.keybind"));
        ...
        if (SoundboardConfig.data.isSyncAudio()) {
            ...
            sliderRow.child(UIContainers.horizontalFlow(Sizing.fixed(0), Sizing.content()));
        } else {
        ...
        timelineSlider = UIComponents.slider(Sizing.fill()).message(v -> {
            ...
            if (file == null) return Component.literal("0:00.0 / 0:00.0");
            ...
            if (durationMs > 0) {
                ...
                return Component.literal(GuiTools.formatTimeMillis(passedMs) + " / -" + GuiTools.formatTimeMillis(Math.max(0, durationMs - passedMs)));
            }
            return Component.literal(GuiTools.formatTimeMillis(Math.max(0, passedMs)) + " / -0:00.0");
        });
        ...
        var leftCtrl   = (FlowLayout) UIContainers.horizontalFlow(Sizing.fixed(80), Sizing.content()).horizontalAlignment(HorizontalAlignment.LEFT);
        var centerCtrl = (FlowLayout) UIContainers.horizontalFlow(Sizing.expand(), Sizing.content()).gap(5).horizontalAlignment(HorizontalAlignment.CENTER);
        var rightCtrl  = (FlowLayout) UIContainers.horizontalFlow(Sizing.fixed(80), Sizing.content()).horizontalAlignment(HorizontalAlignment.RIGHT);
        ...
        loopBtn.tooltip(Component.translatable("gui.opensoundboard.loop"));
        loopBtn.sizing(Sizing.fixed(30), Sizing.content());

        setStartBtn = UIComponents.button(Component.translatable("gui.opensoundboard.set_start"), b -> {
            if (selectedFile == null) return;
            ...
            if (client.player != null)
                client.player.sendMessage(Component.translatable("message.opensoundboard.start_point_set", selectedFile.getName()), true);
        });
        setStartBtn.sizing(Sizing.fixed(80), Sizing.content());
        setStartBtn.tooltip(Component.translatable("tooltip.opensoundboard.set_start"));
    ...
    private ButtonComponent buildToolbarButton(String labelKey, String tooltipKey, int width, java.util.function.Consumer<ButtonComponent> action) {
        var btn = UIComponents.button(Component.translatable(labelKey), action);
        btn.sizing(Sizing.fixed(width), Sizing.content());
        btn.tooltip(Component.translatable(tooltipKey));
        return btn;
    }
    ...
    private ButtonComponent buildControlButton(String symbol, String tooltipKey, java.util.function.Consumer<ButtonComponent> action) {
        var btn = UIComponents.button(Component.literal(symbol), action);
        btn.tooltip(Component.translatable(tooltipKey));
    ...
    private static Component sortModeLabel() {
        return Component.translatable("gui.opensoundboard.sort." + SoundboardConfig.data.getSortMode()).withStyle(ChatFormatting.AQUA);
    }

    private static Component sortDirLabel() {
        return Component.literal(SoundboardConfig.data.isSortAscending() ? "▲" : "▼").withStyle(ChatFormatting.AQUA);
    }
    ...
    private void scanSounds() {
        String query = queryField.getValue().trim().toLowerCase();
    ...
    private FlowLayout buildFolderRow(File dir) {
        ...
        var nameLabel = UIComponents.label(Component.literal(label).withStyle(ChatFormatting.YELLOW));
        nameLabel.horizontalTextAlignment(HorizontalAlignment.LEFT).horizontalSizing(Sizing.expand());

        var openBtn = UIComponents.button(Component.literal("▶"), b -> {});
        openBtn.sizing(Sizing.fixed(PLAY_WIDTH), Sizing.content());
        openBtn.active(false);

        row.child(openBtn);
        row.child(UIContainers.horizontalFlow(Sizing.fixed(FAV_WIDTH), Sizing.content()));
        row.child(nameLabel);
    ...
    private FlowLayout buildBackRow() {
        ...
        var backBtnIcon = UIComponents.button(Component.literal("←"), b -> {});
        backBtnIcon.sizing(Sizing.fixed(PLAY_WIDTH), Sizing.content());
        backBtnIcon.active(false);

        var nameLabel = UIComponents.label(
                Component.literal("← " + (currentFolder != null ? currentFolder.getName() : "")).withStyle(ChatFormatting.GRAY));
        nameLabel.horizontalTextAlignment(HorizontalAlignment.LEFT).horizontalSizing(Sizing.expand());
    ...
    private FlowLayout buildRow(File file) {
        ...
        favBtn.tooltip(Component.translatable("gui.opensoundboard.favorite"));
        favBtn.sizing(Sizing.fixed(FAV_WIDTH), Sizing.content());

        var nameLabel = UIComponents.label(Component.empty());
        nameLabel.horizontalTextAlignment(HorizontalAlignment.LEFT);

        var playBtn = UIComponents.button(Component.translatable("gui.opensoundboard.play"), b -> {
            selectSound(file);
            togglePlay(file);
            refreshListVisuals();
        });
    ...
    private void updateRowVisuals(File file, FlowLayout row) {
        ...
        w.nameLabel().text(Component.literal(
                GuiTools.trimName(minecraft.gameRenderer, GuiTools.baseName(file), listWidth - FAV_WIDTH - PLAY_WIDTH - 20)
        ).formatted(isPlaying ? ChatFormatting.YELLOW : ChatFormatting.WHITE));
        w.playBtn().setMessage(isPlaying
                ? Component.translatable("gui.opensoundboard.stop").withStyle(ChatFormatting.RED)
                : Component.translatable("gui.opensoundboard.play"));
    }
    ...
    private void selectSound(File file) {
        ...
        if (file == null) {
            detailLabel.text(Component.translatable("gui.opensoundboard.select_hint").withStyle(ChatFormatting.GRAY));
            detailLocalSlider.active(false);
            if (detailPlayerSlider != null) detailPlayerSlider.active(false);
            detailBindBtn.active(false);
            detailBindBtn.setMessage(Component.translatable("gui.opensoundboard.keybind.none"));
            setPlaybackControlsActive(false);
            timelineSlider.value(0);
            if (!timeField.isFocused()) timeField.text("0:00.0");
        } else {
            var data = SoundboardConfig.get(file.getName());
            detailLabel.text(Component.translatable("gui.opensoundboard.settings_for", file.getName()).withStyle(ChatFormatting.YELLOW));
            ...
            if (playing) {
                ...
                if (!timeField.isFocused())
                    timeField.text(GuiTools.formatTimeMillis(SoundboardAudioSystem.getTimeMillis(file.getName())));
            } else {
                timelineSlider.value(0);
                if (!timeField.isFocused()) timeField.text("0:00.0");
            }
            pauseBtn.setMessage(SoundboardAudioSystem.isPaused(file.getName()) ? Component.literal("▶") : Component.literal("⏸"));
        }
        refreshListVisuals();
    }
    ...
    public boolean keyPressed(net.minecraft.client.input.KeyEvent input) {
        ...
        if (isBinding && selectedFile != null) {
            ...
            detailBindBtn.setMessage(GuiTools.keyBindLabel(pendingKeybind).copy().withStyle(ChatFormatting.YELLOW));
            return true;
        }

...
public class SoundboardConfigScreen extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout root) {
        ...
        root.child(Components.label(Text.translatable("title.opensoundboard.config").formatted(Formatting.BOLD))
                .horizontalTextAlignment(HorizontalAlignment.CENTER)
                .horizontalSizing(Sizing.fixed(columnWidth))
                .margins(Insets.bottom(8)));

        var panel = (FlowLayout) Containers.verticalFlow(Sizing.fixed(columnWidth), Sizing.content())
                .gap(6)
                .padding(Insets.of(6))
        ...
        globalVolumeSection = Containers.verticalFlow(Sizing.fill(100), Sizing.content()).gap(4);
        ...
        buttons.child(Components.button(Text.translatable("gui.done"), b -> returnToParentAndApply())
                .sizing(Sizing.fixed(140), Sizing.content()));

        buttons.child(Components.button(Text.translatable("gui.cancel"), b -> this.client.setScreen(parent))
                .sizing(Sizing.fixed(140), Sizing.content()));
    ...
    private void rebuildGlobalVolumeSection() {
        ...
        if (SoundboardConfig.data.isSyncGlobalVolume()) {
            var slider = Components.slider(Sizing.fill(100));
            ...
            var localSlider = Components.slider(Sizing.fill(100));
            ...
            var playerSlider = Components.slider(Sizing.fill(100));
    ...
    private FlowLayout sliderRow(Text label, SliderComponent slider, Text tooltip) {
        var wrap = Containers.verticalFlow(Sizing.fill(100), Sizing.content()).gap(2);
        var lbl = Components.label(Text.literal(label.getString()).formatted(Formatting.GRAY));
        lbl.tooltip(tooltip);
        wrap.child(lbl);
        wrap.child(slider);
        return wrap;
    ...
    private FlowLayout cycleRow(Text label, int initialValue, Text tooltip) {
        ...
        var lbl = Components.label(label);
        lbl.tooltip(tooltip);
    ...
    private FlowLayout keybindModeRow() {
        ...
        var lbl = Components.label(Text.translatable("option.opensoundboard.keybindMode"));
    ...
    private FlowLayout toggleRow(Text label, boolean initial, BoolConsumer onChange, Text tooltip) {
        ...
        var toggle = Components.button(toggleLabel(initial), b -> {
